...
struct UpsampleLayer : torch::nn::Module
{
    ...
    torch::Tensor forward(torch::Tensor x) {
        ...
        torch::ArrayRef<int64_t> sizes = x.sizes();
...
void Darknet::create_modules()
{
	...
	for (size_t i = 0, len = blocks.size(); i < len; i++)
	{
		...
		if (layer_type == "convolutional")
		{
			...
			{
				torch::nn::BatchNormImpl bn = torch::nn::BatchNormImpl(bn_options(filters));	//@ihmc3jn09hk Fix for PyTorch 1.6
                		module->push_back(bn);
			}
...
void Darknet::load_weights(const char *weight_file)
{
	...
	for (size_t i = 0; i < module_list.size(); i++)
	{
		...
		if (batch_normalize > 0)
		{
			...
			torch::nn::BatchNormImpl *bn_imp = dynamic_cast<torch::nn::BatchNormImpl *>(bn_module.get()); //@ihmc3jn09hk Fix for PyTorch 1.6
